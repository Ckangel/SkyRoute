<<<<<<< HEAD
   ...
   const API_KEY = '140d6fa8dc6e9d8e4e4905f4b1d95da4'
   // Fetch flight route notification from Aviation Stack API
   async function fetchFlightRoutes() {
       try {
           const response = await fetch(`https://api.aviationstack.com/v1/flights?access_key=${API_KEY}&limit=1`);
           const data = await response.json();
           if (data.data && data.data.length > 0) {
               const route = data.data[0];
               document.getElementById('flightNotification').innerHTML = `
                   <p>Latest Route: ${route.departure.airport} (${route.departure.airport}) to 
                   ${route.arrival.airport} (${route.arrival.airport}) operated by ${route.airline.name}</p>
               `;
           } else {
...
const API_KEY = "ca5985d8cb9ace73bffc1909b432f952";

// Fetch flight route notification from Aviation Stack API
async function fetchFlightRoutes() {
  try {
    const response = await fetch(
      `http://api.aviationstack.com/v1/flights? access_key=${API_KEY}&limit=1`
    );
    const data = await response.json();
    if (data.data && data.data.length > 0) {
      const route = data.data[0];
    ...
    console.error("Error fetching flight routes:", error);
    document.getElementById("flightNotification").innerHTML =
      "<p>Error loading flight route information.</p>";
  }
}
...
function sendBookingEmail(formData) {
  ...
  const hotelName =
    Array.from(document.getElementById("hotel").options).find(
      (opt) => opt.value === hotel
    )?.text || hotel;
  const originName =
    Array.from(document.getElementById("origin").options).find(
...
fetchFlightRoutes();
populateDestinations();
>>>>>>> f783744a0bdbac85cc4cdf0c21a152b8696f4d9a
